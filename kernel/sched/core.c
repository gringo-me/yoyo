...
static int __migrate_task(struct task_struct *p, int src_cpu, int dest_cpu)
{
	...
	/*	if(p->futex != NULL && p->waiter != NULL){
		...
		set_task_cpu(p, dest_cpu);
			...
			int new_prio = p->smp_prio[dest_cpu];
	//		       param.sched_priority = p->pi_cpu_list[dest_cpu];
			       __setscheduler(rq_dest, p, SCHED_FIFO, param.sched_priority);
			       printk(KERN_EMERG "migrate_task: task %d from %d to  %d old priority %d new priority %d\n",p->pid ,src_cpu,dest_cpu,old_prio, p->rt_priority);
			}
